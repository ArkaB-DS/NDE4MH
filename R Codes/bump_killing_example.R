...
one_sample <- function(len = 1e5)
{
  ...
  for (i in 2:len)
  {
    y <- rskewnorm(1,0, 0.54, shape = 10)
    if(runif(1) < dlnorm(y,0,1)/dlnorm(x[i-1],0,1)*
                  dskewnorm(x[i-1],0,0.54,10)/dskewnorm(y,0,0.54,10)
    )
    {
      x[i] <- y
      prob[i]<-1
...
pdf("bump_killing.pdf", height = 5, width = 10)
par(mfrow=c(1,2))
ts.plot(x$mc[5e4:1e5], ylab="MC")
plot(density(x$mc), ylab = expression(hat(f[h])), xlab = "x",
     main = "", lty = 2, ylim = c(0, 1), col = "black")
lines(density(rlnorm(1e5, 0, 1)), col = "red")
lines(density(x$mc, bw = h_mh), col = "blue", lty = 4)
lines(grid, vals, type = "l", col = "seagreen", lty = 5)
legend("topright", legend = c("true density", "KDE(h_iid)",
                              "KDE(h_mh)", "KDE(h_bk)"),
       lty = c(1,2,4,5), col = c("red", "black", "blue", "seagreen"),
       cex = 0.8)
dev.off()
